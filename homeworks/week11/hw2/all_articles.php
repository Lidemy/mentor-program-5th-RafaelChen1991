<?php
  ...
  $stmt->bind_param('sii', $name, $items_per_page, $offset);
  $result = $stmt->execute();
  if(!$result) {
...
<body>
      ...
      <div class="admin-posts">
        <div class="admin-post">
          ...
          <div class="admin-post__info"><div class="admin-post__created-at"><?php echo escape($row['last_edited_at']); ?></div>
            <a class="admin-post__btn" href="blog.php?name=<?php echo $name; ?>&id=<?php echo escape($row['id']); ?>">
              read more
            </a>
          </div>
        </div>
  ...
  <div class="paginator">
    <?php if ($page !== 1) { ?>
      <a href="all_articles.php?name=<?php echo escape($name);?>&page=1">首頁</a>
      <a href="all_articles.php?name=<?php echo escape($name);?>&page=<?php echo $page - 1; ?>">上一頁</a>
    <?php } ?>
    <?php if ($page !== $total_page) { ?>
      <a href="all_articles.php?name=<?php echo escape($name);?>&page=<?php echo $page + 1; ?>">下一頁</a>
      <a href="all_articles.php?name=<?php echo escape($name);?>&page=<?php echo $total_page; ?>">末頁</a>
    <?php } ?>
  </div>
